...
> };
>
> 3.下面的代码调用 \_client。GetChatCompletionsAsync，并将模型名称
> （\_modelName） 和 options 变量作为参数，并将结果存储在名为
> completions 的 ChatCompletion 类型的变量中。它将完成的内容作为
> SummarizeAsync 方法的结果作为字符串返回。
>
