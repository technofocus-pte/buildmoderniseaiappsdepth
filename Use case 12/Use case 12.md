...
1.  登录到 **Azure portal**- 使用 Azure
    登录凭据+++[https://portal.azure.com+++](https://portal.azure.com+++/)。

2.  单击 **Subscriptions** 并选择 **Settings** 下的 **Resource
    Providers**。+++[https://portal.azure.com+++](https://portal.azure.com+++/)
...
1.  打开浏览器，转到
    +++[https://portal.azure.com+++](https://portal.azure.com+++/)，然后使用
    Azure 订阅帐户登录。
...
\[！注意\] **注意：**如果您看到一条消息，指出 Azure OpenAI
...
> 5\. 在 **Deploy model** 对话框中，设置以下内容，然后选择
> **Create**以部署模型。
...
1.  SELECT azure_ai.set_setting('azure_openai.endpoint','{endpoint}');

2.  SELECT azure_ai.set_setting('azure_openai.subscription_key',
    '{api-key}');
...
4.  SELECT azure_ai.get_setting('azure_openai.endpoint');

5.  SELECT azure_ai.get_setting('azure_openai.subscription_key');
...
\[！注意\] **注意：**在执行以下命令之前，请连接到 psql 命令提示符。
...
5.  在 Select Setup Install Mode 选项卡上，选择 **Install only for me
    （recommended）**![A screenshot of a computer AI-generated content
    may be incorrect.](./media/image93.jpeg)
